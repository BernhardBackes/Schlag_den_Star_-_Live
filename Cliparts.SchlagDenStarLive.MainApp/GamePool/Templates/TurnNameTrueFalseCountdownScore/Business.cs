...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.TurnNameTrueFalseCountdownScore {
    ...
    public class DatasetContent : Cliparts.Messaging.Message, INotifyPropertyChanged {
        ...
        public string TopText {
            ...
            set {
                if (this.topText != value) {
                    if (value == null) this.topText = string.Empty;
                    else this.topText = value;
                    this.on_PropertyChanged();
                }
            }
        }

        private string taskFirstname = string.Empty;
        public string TaskFirstname {
            get { return this.taskFirstname; }
            set {
                if (this.taskFirstname != value) {
                    if (value == null) this.taskFirstname = string.Empty;
                    else this.taskFirstname = value;
                    this.buildToString();
                    this.on_PropertyChanged();
                }
            }
        ...
        private string solutionFirstname = string.Empty;
        public string SolutionFirstname {
            get { return this.solutionFirstname; }
            set {
                if (this.solutionFirstname != value) {
                    if (value == null) this.solutionFirstname = string.Empty;
                    else this.solutionFirstname = value;
                    this.on_PropertyChanged();
                }
            }
        }
        ...
        public string Surname {
            ...
            set {
                if (this.surname != value) {
                    if (value == null) this.surname = string.Empty;
        ...
        public bool TaskIsTrue {
            get {
                return string.IsNullOrEmpty(this.SolutionFirstname) || this.TaskFirstname == this.SolutionFirstname;
            }
        }
        ...
        private void buildToString() { this.toString = string.Format("{0} {1}", this.TaskFirstname, this.Surname); }
    ...
    public class Business : _Base.Score.Business {
        ...
        private int questionIDThreshold = 20;
        public int QuestionIDThreshold {
            get { return this.questionIDThreshold; }
            set {
                if (this.questionIDThreshold != value) {
                    if (value < 1) value = 1;
                    if (!this.SampleIncluded &&
                        value < 2) value = 2;
                    this.questionIDThreshold = value;
                    this.on_PropertyChanged();
                }
            }
        }
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.TurnNameTrueFalseCountdownScore'", typeIdentifier);
        }
        ...
        public void Resolve() {
            if (this.SelectedDataset is DatasetContent &&
                this.GetDatasetID(this.SelectedDatasetIndex) > 0) {
                if ((this.LeftPlayerSelection == SelectionValues.True && this.SelectedDataset.TaskIsTrue) ||
                    (this.LeftPlayerSelection == SelectionValues.False && !this.SelectedDataset.TaskIsTrue)) this.LeftPlayerScore++;
                if ((this.RightPlayerSelection == SelectionValues.True && this.SelectedDataset.TaskIsTrue) ||
        ...
        public int GetDatasetID(
            int index){
            if (this.SampleIncluded) return index;
            else return index + 1;
        }
                ...
                try {
                    ...
                    while ((line = file.ReadLine()) != null) {
                        ...
                        if (!string.IsNullOrEmpty(trimLine)) {
                            lineCounter++;
                            if (newDataset is DatasetContent) {
                                if (lineCounter == 2) newDataset.TaskFirstname = trimLine;
                                else if (lineCounter == 3) newDataset.Surname = trimLine;
                                else if (lineCounter == 4) newDataset.SolutionFirstname = trimLine;
                            }
                            else newDataset = new DatasetContent(trimLine);
                        }
        ...
        internal void Vinsert_SetContent() { this.Vinsert_SetContent(this.insertScene, this.SelectedDataset, this.GetDatasetID(this.SelectedDatasetIndex)); }
        internal void Vinsert_SetContent(
            ...
            if (scene is Insert &&
                ...
                scene.SetTopText(dataset.TopText);
                scene.SetTaskFirstname(dataset.TaskFirstname);
                scene.SetSolutionFirstname(dataset.SolutionFirstname);
                scene.SetSurname(dataset.Surname);
                scene.SetCounterValue(datasetID);
                scene.SetShowCounter(datasetID >= 1 && datasetID <= this.QuestionIDThreshold);
            }
        }
        internal void Vinsert_Resolve() {
            ...
            if (this.insertScene is VRemote4.HandlerSi.Scene && 
                ...
                if (this.SelectedDataset.TaskIsTrue) this.insertScene.ResolveGreen();
                else this.insertScene.ResolveRed();
                this.insertScene.PlayResolve();
            }
        }
        internal void Vinsert_TurnName() {
            if (this.insertScene is VRemote4.HandlerSi.Scene &&
                this.SelectedDataset is DatasetContent &&
                !this.SelectedDataset.TaskIsTrue) this.insertScene.Rotate();
        }
        internal void Vinsert_ContentOut() { if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.ToOut(); }
        internal void Vinsert_PlayPling() { if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.PlayPling(); }
        public override void Vinsert_UnloadScene() { this.insertScene.Unload(); }

        public override void Vhost_LoadScene() { this.hostScene.Load(); }
        public void Vhost_SetContent() { if (this.hostScene is VRemote4.HandlerSi.Scene) this.Vhost_SetContent(this.hostScene, this.SelectedDataset, this.LeftPlayerSelection, this.RightPlayerSelection); }
        public void Vhost_SetContent(
            Host scene,
            DatasetContent dataset,
            SelectionValues leftPlayerSelection,
            SelectionValues rightPlayerSelection) {
            if (scene is Host &&
                dataset is DatasetContent) {
                scene.SetTopText(dataset.TopText);
                scene.SetName(string.Format("{0} {1}", dataset.TaskFirstname, dataset.Surname));
                scene.SetSolution(dataset.SolutionFirstname);
                scene.SetIsTrue(dataset.TaskIsTrue);
        ...
        internal void Vplayer_SetContent(
            VentuzScenes.GamePool.TurnNameTrueFalseCountdownScore.Player scene,
            DatasetContent dataset) {
            if (scene is Player &&
                dataset is DatasetContent) {
                scene.SetTopText(dataset.TopText);
        ...
        internal void Vleftplayer_SetContent() { if (this.leftPlayerScene is VRemote4.HandlerSi.Scene) this.Vplayer_SetContent(this.leftPlayerScene, this.SelectedDataset); }
        internal void Vleftplayer_EnableInput() { if (this.leftPlayerScene is VRemote4.HandlerSi.Scene) this.leftPlayerScene.EnableTouch(); }
        internal void Vleftplayer_DisableInput() { if (this.leftPlayerScene is VRemote4.HandlerSi.Scene) this.leftPlayerScene.DisableTouch(); }
        ...
        internal void Vrightplayer_SetContent() { if (this.rightPlayerScene is VRemote4.HandlerSi.Scene) this.Vplayer_SetContent(this.rightPlayerScene, this.SelectedDataset); }
        internal void Vrightplayer_EnableInput() { if (this.rightPlayerScene is VRemote4.HandlerSi.Scene) this.rightPlayerScene.EnableTouch(); }
        internal void Vrightplayer_DisableInput() { if (this.rightPlayerScene is VRemote4.HandlerSi.Scene) this.rightPlayerScene.DisableTouch(); }
        ...
        private void sync_leftPlayerScene_PropertyChanged(object content) {
            ...
            if (e is PropertyChangedEventArgs) {
                if (e.PropertyName == "Selection") {
                    int datasetID = this.GetDatasetID(this.SelectedDatasetIndex);
                    if (datasetID <= this.QuestionIDThreshold) {
