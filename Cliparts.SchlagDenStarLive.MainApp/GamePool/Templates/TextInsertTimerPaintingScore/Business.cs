...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.TextInsertTimerPaintingScore {
    ...
    public class Business : _Base.TimerScore.Business {
        ...
        private VRemote4.HandlerSi.Client.Business paintingClient;
        private Painting ventuzPaintingScene;
        public VRemote4.HandlerSi.Scene.States VentuzPaintingSceneStatus
        {
            get
            {
                if (this.ventuzPaintingScene is Painting) return this.ventuzPaintingScene.Status;
                else return VRemote4.HandlerSi.Scene.States.Unloaded;
            }
        }
        private string paintingClientHostname = string.Empty;
        public string PaintingClientHostname
        {
            get { return this.paintingClientHostname; }
            set
            {
                if (this.paintingClientHostname != value)
                {
                    if (value == null) value = string.Empty;
                    this.paintingClientHostname = value;
                    this.on_PropertyChanged();
                }
            }
        }
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.TextInsertTimerPaintingScore'", typeIdentifier);
        }
        ...
        public override void Pose(
            ...
            if (this.localVentuzHandler.TryAddClient("Painting", false, out this.paintingClient))
            {
                this.paintingClient.HostnameChanged += this.paintingClient_HostnameChanged;
                this.paintingClient.StatusChanged += this.paintingClient_StatusChanged;
                this.ventuzPaintingScene = new Painting(syncContext, this.paintingClient, 0);
                this.ventuzPaintingScene.StatusChanged += this.ventuzPaintingScene_StatusChanged;
            }
        ...
        internal void True()
        {
            if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.PlayJingleTrue();
        ...
        internal void False()
        {
            if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.PlayJingleFalse();
        }
        ...
        internal void NextTeam()
        {
            ...
            {
                case PlayerSelection.RightPlayer:
                    this.SelectedPlayer = PlayerSelection.LeftPlayer;
                    break;
                default:
                    this.SelectedPlayer = PlayerSelection.RightPlayer;
                    break;
            }
        }
                ...
                try
                {
                    ...
                    while ((line = file.ReadLine()) != null)
                    {
                        if (!string.IsNullOrEmpty(line))
                            this.tryAddDataset(new DatasetContent(line.Trim()), -1);
                    }
                    file.Close();
                }
                catch (Exception exc)
        ...
        public void Vplayers_Start() { this.paintingClient.Start(this.PaintingClientHostname); }
        public void Vplayers_Init() { if (this.VentuzPaintingSceneStatus == VRemote4.HandlerSi.Scene.States.Available) this.Vplayers_Reset(); }
        internal void Vplayers_Reset() { this.ventuzPaintingScene.Reset(); }
        internal void Vplayers_ContentIn()
        {
            this.Vplayers_SetContent();
            if (this.VentuzPaintingSceneStatus == VRemote4.HandlerSi.Scene.States.Available)
            {
                this.ventuzPaintingScene.ToTextIn();
                this.ventuzPaintingScene.ResetPaint();
            }
        }
        internal void Vplayers_SetContent()
        {
            if (this.ventuzPaintingScene is VRemote4.HandlerSi.Scene && this.SelectedDataset is DatasetContent) this.VPpayers_SetContent(this.ventuzPaintingScene, this.SelectedDataset);
        }
        internal void VPpayers_SetContent(
            Painting scene,
            DatasetContent dataset)
        {
            if (scene is Painting &&
                dataset is DatasetContent)
            {
                scene.SetText(dataset.Text);
            }
        ...
        internal void Vplayers_ContentOut()
        {
            if (this.VentuzPaintingSceneStatus == VRemote4.HandlerSi.Scene.States.Available)
            {
                this.ventuzPaintingScene.ToTextOut();
                this.ventuzPaintingScene.ResetPaint();
            }
        }
        internal void Vplayers_ShutDown() { this.paintingClient.Shutdown(); }
        ...
        void paintingClient_HostnameChanged(object sender, VRemote4.HandlerSi.Client.HostnameArgs e)
        {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_paintingClient_HostnameChanged);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }

        private void sync_paintingClient_HostnameChanged(object content)
        {
            VRemote4.HandlerSi.Client.HostnameArgs e = content as VRemote4.HandlerSi.Client.HostnameArgs;
            if (e is VRemote4.HandlerSi.Client.HostnameArgs) this.PaintingClientHostname = e.Name;
        }

        void paintingClient_StatusChanged(object sender, VRemote4.HandlerSi.Client.ClientStateArgs e)
        {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_paintingClient_StatusChanged);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }
        private void sync_paintingClient_StatusChanged(object content)
        {
            ...
            if (e is VRemote4.HandlerSi.Client.ClientStateArgs &&
                ...
                this.ventuzPaintingScene is VRemote4.HandlerSi.Scene) this.ventuzPaintingScene.Load();
        }

        void ventuzPaintingScene_StatusChanged(object sender, VRemote4.HandlerSi.Scene.StatusArgs e)
        {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_ventuzPaintingScene_StatusChanged);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }
        private void sync_ventuzPaintingScene_StatusChanged(object content)
        {
            ...
            {
                if (e.Status == VRemote4.HandlerSi.Scene.States.Unloaded) this.paintingClient.Shutdown();
                else if (e.Status == VRemote4.HandlerSi.Scene.States.Available) this.Vplayers_Init();
            }
            this.on_PropertyChanged("VentuzPaintingSceneStatus");
        }
