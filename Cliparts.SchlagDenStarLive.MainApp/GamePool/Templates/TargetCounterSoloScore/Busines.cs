...
using Cliparts.SchlagDenStarLive.MainApp.VentuzScenes.GamePool.TargetCounterSoloScore;

namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.TargetCounterSoloScore
{
    ...
    public class Business : _Base.Score.Business {
        ...
        private bool leftBuzzerLocked = false;
        public bool LeftBuzzerLocked
        {
            get { return this.leftBuzzerLocked; }
            set
            {
                if (this.leftBuzzerLocked != value)
                {
                    this.leftBuzzerLocked = value;
                    this.on_PropertyChanged();
                }
            }
        }

        private bool rightBuzzerLocked = false;
        public bool RightBuzzerLocked
        {
            get { return this.rightBuzzerLocked; }
            set
            {
                if (this.rightBuzzerLocked != value)
                {
                    this.rightBuzzerLocked = value;
                    this.on_PropertyChanged();
                }
            }
        }
        ...
        private int counterTarget = 10;
        public int CounterTarget
        {
            get { return this.counterTarget; }
            set
            {
                if (this.counterTarget != value)
                {
                    if (value < 1) this.counterTarget = 1;
                    else this.counterTarget = value;
                    this.on_PropertyChanged();
                }
            }
        }
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.TargetCounterSoloScore'", typeIdentifier);
        }
        ...
        public override void ResetData() {
            base.ResetData();
            this.LeftBuzzerLocked = false;
            this.RightBuzzerLocked = false;
        ...
        public virtual void DoCounter(
            ...
            if (this.LeftPlayerCounter < this.CounterTarget &&
                this.RightPlayerCounter < this.CounterTarget)
            {
                switch (player)
                {
                    case PlayerSelection.LeftPlayer:
                        if (!this.LeftBuzzerLocked)
                        {
                            this.LeftBuzzerLocked = true;
                            ...
                            Helper.invokeActionAfterDelay(() => { this.LeftBuzzerLocked = false; }, 2500, this.syncContext);
                        }
                        break;
                    case PlayerSelection.RightPlayer:
                        if (!this.RightBuzzerLocked)
                        {
                            this.RightBuzzerLocked = true;
                            this.RightPlayerCounter++;
                            if (this.RightPlayerCounter >= this.CounterTarget)
                            {
                                this.insertMasterScene.Sampler.Play(VentuzScenes.Insert.Sampler.SampleElements.Good);
                                this.SetRightPlayerOn();
                            }
                            ...
                            Helper.invokeActionAfterDelay(() => { this.RightBuzzerLocked = false; }, 2500, this.syncContext);
                        }
                        break;
                }
            }
            if (this.LeftPlayerCounter >= this.CounterTarget ||
                this.RightPlayerCounter >= this.CounterTarget) this.LockBuzzer();
