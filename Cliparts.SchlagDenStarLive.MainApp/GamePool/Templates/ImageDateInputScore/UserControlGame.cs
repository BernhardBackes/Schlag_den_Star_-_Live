...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.ImageDateInputScore {
    ...
    public partial class UserControlGame : _Base.Score.UserControlGame {
        ...
        public void Pose(
            ...
            bind = new Binding("Text", this.business, "LeftPlayerValue");
            bind.Format += (s, e) => { e.Value = Business.DateTimeToText((DateTime)e.Value); };
            this.textBoxLeftPlayerValue.DataBindings.Add(bind);

            bind = new Binding("Text", this.business, "LeftPlayerDifference");
            bind.Format += (s, e) => { e.Value = e.Value.ToString(); };
            this.textBoxLeftPlayerDifference.DataBindings.Add(bind);

            bind = new Binding("Text", this.business, "RightPlayerValue");
            bind.Format += (s, e) => { e.Value = Business.DateTimeToText((DateTime)e.Value); };
            this.textBoxRightPlayerValue.DataBindings.Add(bind);
        ...
        protected override void Dispose(bool disposing) {
            ...
            this.textBoxLeftPlayerValue.DataBindings.Clear();
            this.textBoxLeftPlayerDifference.DataBindings.Clear();
            this.textBoxRightPlayerValue.DataBindings.Clear();
        ...
        private void setDatasetItems() {
            if (this.selectedDataset is DatasetContent) {
                this.pictureBoxTask.Image = this.selectedDataset.TaskThumb;
                this.textBoxSolution.Text = Business.DateTimeToText(this.selectedDataset.Solution);
            }
            else {
                this.pictureBoxTask.Image = null;
        ...
        private void textBoxLeftPlayerValue_Click(object sender, EventArgs e) {
            DateTime startValue = DateTime.Today;
            if (this.business.LeftPlayerValue > Business.ZeroTime) startValue = this.business.LeftPlayerValue;
            _Base.FormPickDateTime form = new _Base.FormPickDateTime(startValue);
            form.BackColor = this.BackColor;
            switch (form.ShowDialog()) {
                case DialogResult.OK:
                    this.business.LeftPlayerValue = form.Value;
                    break;
            }
            form.Dispose();
        }

        private void textBoxRightPlayerValue_Click(object sender, EventArgs e) {
            DateTime startValue = DateTime.Today;
            if (this.business.RightPlayerValue > Business.ZeroTime) startValue = this.business.RightPlayerValue;
            _Base.FormPickDateTime form = new _Base.FormPickDateTime(startValue);
            form.BackColor = this.BackColor;
            switch (form.ShowDialog()) {
                case DialogResult.OK:
                    this.business.RightPlayerValue = form.Value;
                    break;
            }
            form.Dispose();
        }
        ...
        private void buttonVinsert_Difference_Click(object sender, EventArgs e) { this.business.Vinsert_DifferencesIn(); }
