...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.TrueOrFalse {
    ...
    public class Business : _Base.Score.Business {
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.TrueOrFalse'", typeIdentifier);
        }
        ...
        private void rateLeftPlayerInput() {
            if (this.SelectedDataset is DatasetContent) {
                switch (this.SelectedDataset.Solution) {
                    case SolutionStates.True:
                        this.LeftPlayerCorrectInput = this.LeftPlayerInput == SelectionStates.True;
                        break;
                    case SolutionStates.False:
                        this.LeftPlayerCorrectInput = this.LeftPlayerInput == SelectionStates.False;
                        break;
                    default:
        ...
        private void rateRightPlayerInput() {
            if (this.SelectedDataset is DatasetContent) {
                switch (this.SelectedDataset.Solution) {
                    case SolutionStates.True:
                        this.RightPlayerCorrectInput = this.RightPlayerInput == SelectionStates.True;
                        break;
                    case SolutionStates.False:
                        this.RightPlayerCorrectInput = this.RightPlayerInput == SelectionStates.False;
                        break;
                    default:
            ...
            if (scene is Game) {
                if (dataset is DatasetContent) {
                    switch (dataset.Solution) {
                        case SolutionStates.True:
                            scene.SetSolution(SelectionStates.True);
                            break;
                        case SolutionStates.False:
                            scene.SetSolution(SelectionStates.False);
                            break;
                    }
                }
        ...
        private void sync_leftPlayerScene_PropertyChanged(object content) {
            ...
            if (e is PropertyChangedEventArgs) {
                if (e.PropertyName == "SelectionIndex") {
                    ...
                    if (selectionIndex == 1) {
                        this.LeftPlayerInput = SelectionStates.True;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                    else if (selectionIndex == 2) {
                        this.LeftPlayerInput = SelectionStates.False;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                }
            }
        ...
        private void sync_rightPlayerScene_PropertyChanged(object content) {
            ...
            if (e is PropertyChangedEventArgs) {
                if (e.PropertyName == "SelectionIndex") {
                    ...
                    if (selectionIndex == 1) {
                        this.RightPlayerInput = SelectionStates.True;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                    else if (selectionIndex == 2) {
                        this.RightPlayerInput = SelectionStates.False;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                }
            }
