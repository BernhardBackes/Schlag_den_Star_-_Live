...
using Cliparts.SchlagDenStarLive.MainApp.VentuzScenes.GamePool.SelectAnswerScore;

namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.SelectAnswerScore {
    ...
    public class DatasetContent : Messaging.Message, INotifyPropertyChanged {
        ...
        public string MovieFilename {
            ...
            set {
                if (this.movieFilename != value) {
                    ...
                    this.Movie = Helper.getThumbnailFromMediaFile(value, 1.8f);
                }
            }
        }
        ...
        [XmlIgnore]
        public bool ValidAudio { get { return File.Exists(this.audioFilename); } }
        ...
        private Business.SolutionStates solution;

        public Business.SolutionStates Solution {
        ...
        private int creditsPositionY = -400;
        ...
        public DatasetContent(
            ...
            if (string.IsNullOrEmpty(pictureFilename)) {
                ...
                string[] textSplit = this.Name.Split(new string[] { "oder" }, StringSplitOptions.RemoveEmptyEntries);
                if (textSplit.Length == 2) {
                    this.LeftText = textSplit[0].Trim();
                    this.RightText = textSplit[1].Trim();
                }
    ...
    public class Business : _Base.Score.Business {
        ...
        private Host.PlayerSelections leftPlayerInput;
        [NotSerialized]
        public Host.PlayerSelections LeftPlayerInput {
            get { return this.leftPlayerInput; }
            set {
                if (this.leftPlayerInput != value) {
                    this.leftPlayerInput = value;
                    if (this.BuzzerMode && value != Host.PlayerSelections.NotAvailable) this.doBuzzer(PlayerSelection.LeftPlayer);
                    this.on_PropertyChanged();
        ...
        private Host.PlayerSelections rightPlayerInput;
        [NotSerialized]
        public Host.PlayerSelections RightPlayerInput {
            get { return this.rightPlayerInput; }
            set {
                if (this.rightPlayerInput != value) {
                    this.rightPlayerInput = value;
                    if (this.BuzzerMode && value != Host.PlayerSelections.NotAvailable) this.doBuzzer(PlayerSelection.RightPlayer);
                    this.on_PropertyChanged();
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.SelectAnswerScore'", typeIdentifier);
        }
        ...
        public override void ResetData() {
            ...
            this.TaskCounter = 0;
            this.SelectDataset(0);
            this.LeftPlayerInput = Host.PlayerSelections.NotAvailable;
            this.RightPlayerInput = Host.PlayerSelections.NotAvailable;
        ...
        private void rateLeftPlayerInput() {
            if (this.SelectedDataset is DatasetContent) {
                switch (this.SelectedDataset.Solution) {
                    case SolutionStates.Left:
                        this.LeftPlayerCorrectInput = this.LeftPlayerInput == Host.PlayerSelections.Left;
                        break;
                    case SolutionStates.Right:
                        this.LeftPlayerCorrectInput = this.LeftPlayerInput == Host.PlayerSelections.Right;
                        break;
                    default:
        ...
        private void rateRightPlayerInput() {
            if (this.SelectedDataset is DatasetContent) {
                switch (this.SelectedDataset.Solution) {
                    case SolutionStates.Left:
                        this.RightPlayerCorrectInput = this.RightPlayerInput == Host.PlayerSelections.Left;
                        break;
                    case SolutionStates.Right:
                        this.RightPlayerCorrectInput = this.RightPlayerInput == Host.PlayerSelections.Right;
                        break;
                    default:
        ...
        private void doBuzzer(
            Content.Gameboard.PlayerSelection buzzeredPlayer) {
            switch (buzzeredPlayer) {
                case PlayerSelection.LeftPlayer:
                    if (this.RightPlayerInput == Host.PlayerSelections.NotAvailable) {
                        this.Vrightplayer_ResetInput();
                        this.Vinsert_BuzzerBorder(buzzeredPlayer);
                    }
                    else {
                ...
                case PlayerSelection.RightPlayer:
                    if (this.LeftPlayerInput == Host.PlayerSelections.NotAvailable) {
                        this.Vleftplayer_ResetInput();
                        this.Vinsert_BuzzerBorder(buzzeredPlayer);
                    }
                    else {
        ...
        public void Vinsert_BorderIn() {
            if (this.insertScene is VRemote4.HandlerSi.Scene &&
                this.insertScene.Border is VentuzScenes.GamePool._Modules.Border) {
        ...
        public void Vinsert_BuzzerBorder(
                ...
                if (this.insertScene is VRemote4.HandlerSi.Scene &&
                    ...
                    switch (buzzeredPlayer) {
                        case PlayerSelection.LeftPlayer:
                            ...
                            if (this.insertScene.Timeout is VentuzScenes.GamePool._Modules.Timeout) this.insertScene.Timeout.BuzzerSoundLeft();
                            break;
                        case PlayerSelection.RightPlayer:
                            this.insertScene.Border.SetBuzzerRight();
                            if (this.insertScene.Timeout is VentuzScenes.GamePool._Modules.Timeout) this.insertScene.Timeout.BuzzerSoundRight();
                            break;
        ...
        public void Vinsert_GameIn() {
            ...
            if (this.insertScene is Insert &&
                ...
                this.insertScene.Game.SetSelectionOut();
        ...
        public void Vinsert_SetGame(
            ...
            if (scene is Game) {
                ...
                if (dataset is DatasetContent) {
                    ...
                    scene.SetLeftText(string.Empty);
                    scene.SetRightText(string.Empty);
                }
                scene.SetSolution(Game.SolutionStates.NotAvailable);
            }
        }
        public void Vinsert_GameInputIn() {
            ...
            if (this.insertScene is Insert &&
                this.insertScene.Game is Game) this.insertScene.Game.SelectionToIn();
        }
        ...
        internal void Vinsert_SetGameInput(
            Game scene,
            Host.PlayerSelections leftPlayerSelection,
            Host.PlayerSelections rightPlayerSelection) {
            if (scene is Game) {
                switch (leftPlayerSelection) {
                    case Host.PlayerSelections.Left:
                        scene.SetLeftPlayerSelection(Game.PlayerSelections.Left);
                        break;
                    case Host.PlayerSelections.Right:
                        scene.SetLeftPlayerSelection(Game.PlayerSelections.Right);
                        break;
                    default:
                        scene.SetLeftPlayerSelection(Game.PlayerSelections.NotAvailable);
                        break;
                }
                switch (rightPlayerSelection) {
                    case Host.PlayerSelections.Left:
                        scene.SetRightPlayerSelection(Game.PlayerSelections.Left);
                        break;
                    case Host.PlayerSelections.Right:
                        scene.SetRightPlayerSelection(Game.PlayerSelections.Right);
                        break;
                    default:
                        scene.SetRightPlayerSelection(Game.PlayerSelections.NotAvailable);
                        break;
                }
            }
        }
            ...
            if (scene is Game) {
                if (dataset is DatasetContent) {
                    switch (dataset.Solution) {
                        case SolutionStates.Left:
                            scene.SetSolution(Game.SolutionStates.Left);
                            break;
                        case SolutionStates.Right:
                            scene.SetSolution(Game.SolutionStates.Right);
                            break;
                        default:
                            scene.SetSolution(Game.SolutionStates.NotAvailable);
                            break;
                ...
                else scene.SetSolution(Game.SolutionStates.NotAvailable);
            }
        }
        public void Vinsert_GameOut() { if (this.insertScene is Insert && this.insertScene.Game is Game) this.insertScene.Game.ToOut(); }
        ...
        public void Vfullscreen_ContentIn() {
            if (this.fullscreenScene is VRemote4.HandlerSi.Scene &&
                ...
                if (!string.IsNullOrEmpty(this.SelectedDataset.Credits)) {
                    this.fullscreenScene.SetCreditsText(this.SelectedDataset.Credits);
                    this.fullscreenScene.SetCreditsPositionY(this.SelectedDataset.CreditsPositionY);
        ...
        public void Vfullscreen_SetContent(
            ...
            if (scene is Fullscreen &&
                ...
                scene.SetAudioFilename(dataset.AudioFilename);
                scene.SetIsAudio(File.Exists(dataset.AudioFilename));
            }
        }
        public void Vfullscreen_StartAudio() {
            if (this.fullscreenScene is Fullscreen &&
                this.SelectedDataset is DatasetContent &&
                this.SelectedDataset.ValidAudio) {
                this.fullscreenScene.SetAudioFilename(this.SelectedDataset.AudioFilename);
                this.fullscreenScene.PlayAudioFile();
            }
        }
        internal void Vfullscreen_Resolve() { if (this.fullscreenScene is VRemote4.HandlerSi.Scene) this.fullscreenScene.SolutionToIn(); }
        ...
        public void Vhost_Set(
            ...
            if (scene is Host) {
                if (dataset is DatasetContent) {
                    ...
                    scene.SetHeadline(hostText);
                    scene.SetLeftText(dataset.LeftText);
                    scene.SetRightText(dataset.RightText);
                    switch (dataset.Solution) {
                        case SolutionStates.Left:
                            scene.SetSolution(Host.SolutionStates.Left);
                            break;
                        case SolutionStates.Right:
                            scene.SetSolution(Host.SolutionStates.Right);
                            break;
                    }
                }
                switch (leftPlayerSelection) {
                    case Host.PlayerSelections.NotAvailable:
                        scene.SetLeftPlayerSelection(Host.PlayerSelections.NotAvailable);
                        break;
                    case Host.PlayerSelections.Left:
                        scene.SetLeftPlayerSelection(Host.PlayerSelections.Left);
                        break;
                    case Host.PlayerSelections.Right:
                        scene.SetLeftPlayerSelection(Host.PlayerSelections.Right);
                        break;
                }
                switch (rightPlayerSelection) {
                    case Host.PlayerSelections.NotAvailable:
                        scene.SetRightPlayerSelection(Host.PlayerSelections.NotAvailable);
                        break;
                    case Host.PlayerSelections.Left:
                        scene.SetRightPlayerSelection(Host.PlayerSelections.Left);
                        break;
                    case Host.PlayerSelections.Right:
                        scene.SetRightPlayerSelection(Host.PlayerSelections.Right);
                        break;
                }
            }
        }
        ...
        public void Vplayer_SetData(
            ...
            if (scene is Player &&
                dataset is DatasetContent) {
                scene.SetHeadline(dataset.Name);
                scene.SetLeftText(dataset.LeftText);
                scene.SetRightText(dataset.RightText);
            }
        }
        ...
        public void Vplayer_SetInput(
            Player scene,
            VentuzScenes.GamePool.SelectAnswerScore.Host.PlayerSelections selection) {
            if (scene is Player) {
                switch (selection) {
                    case Host.PlayerSelections.NotAvailable:
                        ...
                        break;
                    case Host.PlayerSelections.Left:
                        scene.SetSelectionLeft();
                        break;
                    case Host.PlayerSelections.Right:
                        scene.SetSelectionRight();
                        break;
                }
            }
        }
        ...
        private void sync_leftPlayerScene_PropertyChanged(object content) {
            ...
            if (e is PropertyChangedEventArgs) {
                if (e.PropertyName == "SelectionIndex") {
                    ...
                    if (selectionIndex == 1) {
                        this.LeftPlayerInput = Host.PlayerSelections.Left;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                    else if (selectionIndex == 2) {
                        this.LeftPlayerInput = Host.PlayerSelections.Right;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                }
            }
        ...
        private void sync_rightPlayerScene_PropertyChanged(object content) {
            ...
            if (e is PropertyChangedEventArgs) {
                if (e.PropertyName == "SelectionIndex") {
                    ...
                    if (selectionIndex == 1) {
                        this.RightPlayerInput = Host.PlayerSelections.Left;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                    else if (selectionIndex == 2) {
                        this.RightPlayerInput = Host.PlayerSelections.Right;
                        if (!this.BuzzerMode) this.Vhost_Set();
                    }
                }
            }
