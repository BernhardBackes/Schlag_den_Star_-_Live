...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.Memory {
    ...
    public class Business : _Base.Score.Business {
        ...
        public const int PicturesCount = 15;
        ...
        private bool duellMode = false;
        public bool DuellMode {
            get { return this.duellMode; }
            set {
                if (this.duellMode != value) {
                    this.duellMode = value;
                    this.on_PropertyChanged();
        ...
        private List<Dataset> datalist = new List<Dataset>();
        public Dataset[] DataList {
            get { return this.datalist.ToArray(); }
            private set {
                int index = 0;
                foreach (Dataset item in this.datalist) {
                    if (value is Dataset[] && 
                        value.Length > index &&
                        value[index] is Dataset) {
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.Memory'", typeIdentifier);
        }
        ...
        public override void Pose(
            ...
            if (this.localVentuzHandler.TryAddClient("Gameboard", false, out this.gameboardClient)) {
                ...
                this.ventuzGameboardScene.TouchFailure += this.ventuzGameboardScene_TouchFailure;
            }
        ...
        public override void Dispose() {
            ...
            foreach (Dataset item in this.datalist) item.PropertyChanged -= this.songDataset_PropertyChanged;
            this.datalist.Clear();
        ...
        public void Vfullscreen_SetContent() {
            if (this.fullscreenScene.Game is Game) {
                ...
                foreach (Dataset item in this.datalist) {
                    game.SetPicturefile(item.FirstCardID, item.Filename);
                    game.SetPicturefile(item.SecondCardID, item.Filename);
                }
            }
        }
        ...
        public void Vfullscreen_SetAllSelected() {
            for (int id = 1; id <= CardsCount; id++) this.Vfullscreen_SetSelected(id);
        }
        ...
        public void Vfullscreen_TouchFailure(int cardID) { if (this.fullscreenScene.Game is Game) this.fullscreenScene.Game.InvokeTouchFailure(cardID); }
        ...
        public void Vgameboard_SetContent() {
            if (this.ventuzGameboardScene is Game) {
                ...
                foreach (Dataset item in this.datalist) {
                    game.SetDuellMode(this.DuellMode);
                    game.SetPicturefile(item.FirstCardID, item.Filename);
                    game.SetPicturefile(item.SecondCardID, item.Filename);
                }
            }
        ...
        private void ventuzGameboardScene_TouchFailure(object sender, int e) {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_ventuzGameboardScene_TouchFailure);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }
        private void sync_ventuzGameboardScene_TouchFailure(object content) {
            if (content is int) this.Vfullscreen_TouchFailure((int)content);
        }
