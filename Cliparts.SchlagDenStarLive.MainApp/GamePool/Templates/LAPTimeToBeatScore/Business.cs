...
using Cliparts.SchlagDenStarLive.MainApp.VentuzScenes.GamePool.LAPTimeToBeatScore;
...
namespace Cliparts.SchlagDenStarLive.MainApp.GamePool.Templates.LAPTimeToBeatScore {
    ...
    public class Business : _Base.Score.Business {
        ...
        public int LAPsCount {
            ...
            set {
                if (this.lapsCount != value) {
                    if (value < 0) lapsCount = 0;
                    else this.lapsCount = value;
                    this.on_PropertyChanged();
                    this.LAPsCounter = this.LAPsCounter;
                }
            }
        }

        private int lapsCounter = 0;
        [NotSerialized]
        public int LAPsCounter {
            get { return this.lapsCounter; }
            set {
                if (this.lapsCounter != value) {
                    if (value < 0) this.lapsCounter = 0;
                    else if (value > this.LAPsCount) this.lapsCounter = this.LAPsCount;
                    else this.lapsCounter = value;
                    this.on_PropertyChanged();
                    this.Vinsert_SetTimeToBeatCounter();
                }
            }
        }
        ...
        public Business(
            ...
            this.ClassInfo = string.Format("'{0}' of 'Templates.LAPTimeToBeatScore'", typeIdentifier);
        }
                ...
                if (this.insertScene is VRemote4.HandlerSi.Scene) {
                    if (!this.timeToBeat.HasValue) {
                        if (this.IllegalRound) {
                            if (this.LAPsCounter < this.LAPsCount) {
                                this.insertScene.TimeToBeat.StartTimer();
                                this.IllegalRound = false;
                            }
                            else {
                                this.insertScene.TimeToBeat.StopTimer();
                            }
                            this.insertScene.TimeToBeat.StartTimer();
                            this.LAPsCounter++;
                        }
                        else {
                            if (this.SecondRun) this.insertScene.TimeToBeat.StopTimer();
                            else {
                                this.insertScene.TimeToBeat.GetPreciseTime();
                                this.insertScene.TimeToBeat.StartTimer();
                                this.LAPsCounter++;
                            }
                        }
                    }
                    else if (this.LAPsCounter < this.LAPsCount) {
                        if (this.IllegalRound) {
                            this.insertScene.TimeToBeat.StartTimer();
                            this.IllegalRound = false;
                        }
                        else this.insertScene.TimeToBeat.SetLAPRestart();
                        this.LAPsCounter++;
                    }
        ...
        public void Vinsert_SetTimeToBeat() {
            if (this.insertScene is VRemote4.HandlerSi.Scene) {
                ...
                this.insertScene.TimeToBeat.SetCounter(this.LAPsCounter);
        ...
        public void Vinsert_SetTimeToBeatCounter() {
            if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.TimeToBeat.SetCounter(this.LAPsCounter);
        }
        public void Vinsert_SetIllegal(bool value) {
        ...
        public void Vinsert_StartTimeToBeatFlashing() { if (this.insertScene is VRemote4.HandlerSi.Scene) this.insertScene.TimeToBeat.StartFlashing(); }
        ...
        private void checkLAPTime(
            double value) {
            if (!this.timeToBeat.HasValue) {
                if (this.SecondRun) {
                    this.Vinsert_StartTimeToBeatFlashing();
                    this.timeIsRunning = false;
                }
                else {
                    this.timeToBeat = value;
                    this.Vinsert_ShowTimeToBeatTime();
                    this.on_PropertyChanged("TimeToBeat");
                }
            }
            else if (this.SecondRun) {
                double offset = value - this.timeToBeat.Value;
                if (offset < 0) {
                    this.Vinsert_StartTimeToBeatFlashing();
                    this.timeIsRunning = false;
                }
                else this.Vinsert_ShowOffsetTime((float)offset);
            }
            else {
                if (value < this.timeToBeat.Value) {
                    this.timeToBeat = value;
                    this.Vinsert_ShowTimeToBeatTime();
                    this.on_PropertyChanged("TimeToBeat");
                }
        ...
        private void timeToBeat_PreciseTimeReceived(object sender, EventArgs e) {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_timeToBeat_PreciseTimeReceived);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }
        protected virtual void sync_timeToBeat_PreciseTimeReceived(object content) { this.checkLAPTime(this.insertScene.TimeToBeat.PreciseTime); }

        private void timeToBeat_LAPTimeReceived(object sender, EventArgs e) {
            SendOrPostCallback callback = new SendOrPostCallback(this.sync_timeToBeat_LAPTimeReceived);
            if (this.syncContext != null) this.syncContext.Post(callback, e);
        }
        protected virtual void sync_timeToBeat_LAPTimeReceived(object content) { this.checkLAPTime(this.insertScene.TimeToBeat.LAPTime); }
        ...
        protected virtual void sync_timeToBeat_StopFired(object content) {
            if (this.timeIsRunning &&
                !this.IllegalRound) this.checkLAPTime(this.insertScene.TimeToBeat.CurrentTime);
            this.timeIsRunning = false;
        }
